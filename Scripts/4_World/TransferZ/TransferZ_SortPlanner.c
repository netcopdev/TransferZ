...
class TransferZSortPlanner : TransferZMaintenanceService
{
    ...
    // The free runs depend only on the anchor and the target grid, which does
    // not change while candidates for one anchor are evaluated. Compute them
    // once per anchor instead of once per candidate orientation.
