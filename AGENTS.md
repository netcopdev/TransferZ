...
- Native stack splitting remains DayZ-owned. TransferZ only chooses the destination of the new split entity: the active `D*` (exact cargo, or ground placement for `VICINITY`), else the stack's own immediate cargo container when it has room, else a configured `P*`, else vanilla. A selected `D*` that cannot accept the split falls back to vanilla rather than trying source/P; if routing reaches `P*` and it cannot accept the split, it also falls back to vanilla (`docs/SPLIT_ROUTING.md`).
