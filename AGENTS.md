...
- Native stack splitting remains DayZ-owned. TransferZ only chooses the destination of the new split entity: the active `D*` (exact cargo, or ground placement for `VICINITY`), else a configured `P*`, else the stack's own immediate cargo container when it has room, else vanilla. A selected `D*` or `P*` that cannot accept the split falls back to vanilla, never to the next rule (`docs/SPLIT_ROUTING.md`).
