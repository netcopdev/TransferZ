...
- Container `Unpack` leaves the source/root container in place and recursively flattens all cargo descendants into the destination. Direct loose items move; nested containers are emptied contents-first and the now-empty nested containers also move to the destination.
...
- The Unpack-container modifier (default U) + Left Drag performs exactly the same Unpack semantics as the header/command on the dragged cargo-bearing container: source stays; all cargo descendants flatten into the release target; emptied nested containers move too.
...
- Sort and Stack are server-authoritative container-maintenance operations. Sort may reposition and rotate only direct cargo children through validated native inventory moves. Sort snapshots every original row/column/orientation and first attempts a complete bounded in-cargo plan. Direct native swaps may be used only where their result is synchronously observable; multiplayer/dedicated-server Sort must not plan the asynchronous `SendServerSwap` path. If the final layout fits but no bounded in-cargo path exists, Sort may create TransferZ's dedicated hidden native cargo buffer and move the same `EntityAI` objects through it; it must never use vicinity/ground or arbitrary player inventory as staging. On failure, restore and verify the exact original snapshot before deleting an empty buffer. Never delete a non-empty sort buffer. Stack may merge only items DayZ itself reports as combinable.
