...
- Sort and Stack are server-authoritative container-maintenance operations. Sort may reposition and rotate only direct cargo children through validated native inventory moves. When rearrangement is required, Sort snapshots every original row/column/orientation, may temporarily stage the same `EntityAI` objects through DayZ vicinity to empty the source, preflights final exact placements, and must synchronously restore the exact original snapshot on any failure. No successful or ordinary failed Sort may leave a tracked item in vicinity or at a partial target layout. Stack may merge only items DayZ itself reports as combinable.
