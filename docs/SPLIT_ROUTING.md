...
2. **Preferred destination (`P*`)** — when the source cargo has no usable location, resolve the player's preferred attachment-path destination and place the split there when its exact cargo can accept the new stack.
