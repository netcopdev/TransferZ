...
TransferZ computes the deterministic target packing **once per Sort request** and reuses the same target coordinates for both execution strategies. The recursive in-cargo parking planner also has a candidate-position work budget: it scales with item/grid size but is capped at **131,072 candidate checks**. Exhausting that planning budget is not treated as a corrupt transaction; it simply abandons the expensive in-cargo search and proceeds to the transactional buffer fallback.

TransferZ first tries that bounded in-cargo plan. If the final layout is valid but the source grid cannot provide enough intermediary workspace, Sort falls back to one dedicated `TransferZ_SortBuffer`. The buffer is a hidden, non-interactive, non-physical native cargo entity created server-authoritatively beside the player for only that transaction. Its fixed cargo size is intentionally the hard external-staging limit: if the required transaction cannot stage through this one buffer, TransferZ restores the original snapshot and reports Sort failure. It does not allocate extra buffers, change buffer geometry, or use another staging location. Ground/vicinity and arbitrary player/world containers are never used as Sort workspace.
