...
Planning is entirely virtual. A cloned record set is used while the planner resolves blockers and cycles, so the authoritative original snapshot remains unchanged for rollback verification. The planner uses genuinely free cells inside the **same cargo grid** as temporary parking when a direct target move is blocked. When a target is occupied by one compatible equal-size item of the same class, TransferZ can instead plan DayZ's native atomic inventory swap; this exchanges the two cargo locations without requiring any empty intermediary cell. Cross-class exchanges created by identical-item grouping use ordinary evacuation or the transactional buffer fallback instead, avoiding asynchronous native-swap verification on dedicated servers.
