...
- If an active destination (`D*`) is selected, first try to place the new split entity in its exact cargo, or through DayZ ground placement around the player for `VICINITY`.
- If `D*` is absent or cannot accept it and the stack is in cargo, try the exact immediate source cargo when space is available.
- If the source is unavailable/full and a preferred destination (`P*`) is configured, try `P*`'s exact cargo.
- If none of those routes can accept the split, leave normal DayZ fallback behavior in control.
